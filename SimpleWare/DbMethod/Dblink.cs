    ...
    public class Dblink
    {
        ...
        public DataSet GetDataset(string sql,string tablename)
        {
            try
            {
                ...
                string strConn = Dblink.connstr();
                SqlConnection conn = new SqlConnection(strConn);
                SqlCommand command = new SqlCommand(sql, conn);
                SqlDataAdapter adapter = new SqlDataAdapter();
                adapter.SelectCommand = command;
                int i = adapter.Fill(ds, tablename);
                if (i == 0)
                    return null;
                else
                    return ds;
        ...
        public DataSet GetDatasetReport(string sql,string table)
        {
            ...
            {
                DataSet ds = new DataSet();
                //string strConn = "server=(local);database =Library;integrated security=true";
                string strConn = Dblink.connstr();
                SqlConnection conn = new SqlConnection(strConn);
                SqlCommand command = new SqlCommand(sql, conn);
                SqlDataAdapter adapter = new SqlDataAdapter();
        ...
        internal Boolean ShowDgvInfo(DevComponents.DotNetBar.SuperGrid.SuperGridControl gridView, bool isShowExcle)
        {
            if (gridView.PrimaryGrid.Rows.Count == 0)
                return false;
            //建立Excel对象
            ...
            {
                excel.Cells[1, i + 1] = gridView.PrimaryGrid.Columns[i].HeaderText;
            }
            //填充数据
            try
            {
                ...
                for (int i = 0; i < gridView.PrimaryGrid.Rows.Count; i++)
                {
                    ...
                    {
                        if (gr[j].GetType() == typeof(string))
                        {
                            excel.Cells[i + 2, j + 1] = "'" + gr[j].Value.ToString();
                        }
                        else
                        {
                            excel.Cells[i + 2, j + 1] = gr[j].Value.ToString();
